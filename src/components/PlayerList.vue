...
<template>
  <div class="sidebar">
    ...
    <ul class="player-list">
      <li v-for="player in players" :key="player.id" @click="$emit('select-player', player)">
        <span class="name">{{ player.username }}</span>
        <span class="rank">{{ player.level ? player.level.level : 'Beginner' }}</span>
      </li>
    </ul>
  </div>
</template>
...
h3 { margin: 0; font-size: 1.2rem; color: #38bdf8; }
...
.error { color: #f87171; padding: 0 15px; font-size: 0.8rem; margin-top: 5px;}
...
.player-list li:hover { background: #334155; }
